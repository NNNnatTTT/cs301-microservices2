...
// NOT FOR PROD
async function createAccount({clientID, accountType, openingDate, initialDeposit, currency, branchID}) {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');

    const insertQuery = `
      INSERT INTO accounts.account_list (client_id, account_type, account_status, opening_date, initial_deposit, currency, branch_id)
      VALUES ($1, $2, $3, $4, $5, $6, $7)
      RETURNING id;
    `;

    const values = [clientID, accountType, accountStatus, openingDate, initialDeposit, currency, branchID];
